...
def check_and_close_trades():
    ...
    Closes any trade stuck OPEN for more than 48 hours (phantom/orphan trades).
    """
    open_trades = models.get_open_trades()
    if not open_trades:
        return

    # Auto-close orphan trades stuck open longer than 48 hours
        ...
        try:
            ...
            if (now_utc - opened_at) > timedelta(hours=48):
                models.log_agent("Executor", trade["symbol"],
                                 f"#{trade['id']} stuck OPEN >48h — force-closing as LOSS")
                _close(trade["id"], "LOSS", -trade["risk_usd"], -1.0)
